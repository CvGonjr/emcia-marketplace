# Registro de decisões

Uma decisão por arquivo, numerada e datada. Formato: contexto, decisão, consequência.

**Decisão registrada não se desfaz por esquecimento.** Se uma delas precisar mudar, escreva uma nova que a supersede e marque a antiga — não edite a original. O histórico é o que impede reabrir a mesma discussão em três meses.

| # | Decisão | Estado |
|---|---|---|
| 001 | Dois plugins: núcleo e playbook | firme |
| 002 | O playbook vive no caso, não no plugin | firme |
| 003 | Chat não tem autoridade de escrita | firme |
| 004 | A antítese roda entre P2 e P3a | firme |
| 005 | Camada resolvida por nível | firme |
| 006 | Procedência em três dimensões | **em conflito com CAT-01** |
| 007 | Um repositório Git por caso | firme |
| 008 | O cliente nunca fala com um agente | firme |
| 009 | Habilidades remetem ao método, não o reproduzem | firme |
| 010 | Três zonas de escrita | proposta |
| 011 | Testes negativos antes do caminho feliz | firme |
| 012 | E3 dividido; termo de autonomia no E4 | firme |
